...
class ConnectivityNode(IdentifiedObject):
    ...
    MemberOf_EquipmentContainer = Instance("CPSM.IEC61970.Core.ConnectivityNodeContainer", allow_none=False,
        ...
        if self.Model is not None:
            return [e for e in self.Model.Elements \
                if "%s.%s" % (e.__module__, e.__class__.__name__) == \
                    "CPSM.IEC61970.Core.ConnectivityNodeContainer" ]
        else:
            return []
    ...
    traits_view = View(Tabbed(
            VGroup("UUID", "pathName", "description", "aliasName", "name",
                label="Attributes"),
            VGroup("Model", "Terminals", "MemberOf_EquipmentContainer",
                label="References"),
            dock="tab"),
        id="CPSM.IEC61970.Topology.ConnectivityNode",
