...
ControlAreaTypeKind = Enum("Interchange", "Forecast", "AGC")
...
class ControlArea(PowerSystemResource):
    ...
    EnergyArea = Instance("CIM14r05.LoadModel.EnergyArea",
        transient=True,
    ...
    traits_view = View(Tabbed(
            VGroup("description", "mRID", "pathName", "localName", "aliasName", "name", "pTolerance", "netInterchange", "type",
                label="Attributes"),
            VGroup("Parent", "ModelingAuthoritySet", "OperatingShare", "OperatedBy_Companies", "PSRType", "PsrLists", "ReportingGroup", "OutageSchedule", "Contains_Measurements", "EnergyArea", "ControlAreaGeneratingUnit", "TieFlow", "BusNameMarker", "TopologicalNode",
                label="References", columns=1),
            dock="tab"),
        id="CIM14r05.ControlArea.ControlArea",
        title="ControlArea",
...
class AltGeneratingUnitMeas(Element):
    ...
    AnalogValue = Instance("CIM14r05.Meas.AnalogValue",
        transient=True,
    ...
    def _get_analogvalues(self):
        ...
        if self.Parent is not None:
            return [e for e in self.Parent.Elements \
                if "%s.%s" % (e.__module__, e.__class__.__name__) == \
                    "CIM14r05.Meas.AnalogValue" ]
        else:
            return []

    _analogvalues = Property(fget=_get_analogvalues)

    ControlAreaGeneratingUnit = Instance("CIM14r05.ControlArea.ControlAreaGeneratingUnit",
        transient=True,
    ...
    def _get_controlareageneratingunits(self):
        ...
        if self.Parent is not None:
            return [e for e in self.Parent.Elements \
                if "%s.%s" % (e.__module__, e.__class__.__name__) == \
                    "CIM14r05.ControlArea.ControlAreaGeneratingUnit" ]
        else:
            return []
    ...
    traits_view = View(Tabbed(
            VGroup("priority",
                label="Attributes"),
            VGroup("Parent", "AnalogValue", "ControlAreaGeneratingUnit",
                label="References"),
            dock="tab"),
        id="CIM14r05.ControlArea.AltGeneratingUnitMeas",
...
class AltTieMeas(Element):
    ...
    TieFlow = Instance("CIM14r05.ControlArea.TieFlow",
        transient=True,
        opposite="AltTieMeas",
    ...
    traits_view = View(Tabbed(
            ...
            VGroup("Parent", "AnalogValue", "TieFlow",
                label="References"),
            dock="tab"),
        id="CIM14r05.ControlArea.AltTieMeas",
        title="AltTieMeas",
        buttons=["OK", "Cancel", "Help"],
        resizable=False)
...
class TieFlow(Element):
    ...
    traits_view = View(Tabbed(
            VGroup("positiveFlowIn",
                label="Attributes"),
            VGroup("Parent", "AltTieMeas", "Terminal", "ControlArea",
                label="References"),
            dock="tab"),
        id="CIM14r05.ControlArea.TieFlow",
        title="TieFlow",
        buttons=["OK", "Cancel", "Help"],
        resizable=False)
...
class ControlAreaGeneratingUnit(Element):
    ...
    AltGeneratingUnitMeas = List(Instance("CIM14r05.ControlArea.AltGeneratingUnitMeas"))
    ...
    traits_view = View(Tabbed(
            VGroup("Parent", "GeneratingUnit", "AltGeneratingUnitMeas", "ControlArea",
                label="References"),
            dock="tab"),
        id="CIM14r05.ControlArea.ControlAreaGeneratingUnit",
